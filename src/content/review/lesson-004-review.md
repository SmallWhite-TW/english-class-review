...
lessonId: lesson-004
lessonNo: 4
title: 260411 英文課程複習講義
slug: lesson-004-review
sourcePath: english-courses/lesson-004/records/260411-lesson-004-review.md
published: true
tags: []
---
# 260411 英文課程複習講義
...
### Topic 摘要
課堂一開始先聊早餐、連假怎麼過、是否有出遊，以及最近是否忙於工作與備課。

### 老師補充重點
- `bread` 通常是不可數名詞；如果要表達一整條麵包，會說 `a loaf of bread`。
- 回答假期近況時，除了 `I stayed home`，最好再補一點活動內容，讓回答更完整自然。
...
### Topic 摘要
課堂中提到替其他公司做 training 的情境，老師延伸說明工作場合常用的說法，例如 `training`、`instructor`，以及怎麼描述這種職場教學情境。

### 老師補充重點
- 工作場合常說 `training`，不一定用 `class`。
- 提供訓練的人可以說 `teacher`，但在職場、技術、專業訓練情境下，`instructor` 更自然。
- `language of instruction` 是「教學語言」，可用來說明課程用哪種語言進行。

### 關鍵單字／片語
- `Instructor / instruct`：講師；教導
- `training`：訓練、培訓
...
### Topic 摘要
這是本堂很重要的一段。老師解釋有些字加上 `-s` 之後不只是單複數差異，而是意思也可能改變。

### 老師補充重點
- `people` 可表示「人們」，也可以表示某個民族、國族中的人民。
- `peoples` 常指「多個民族／族群」。
- `time` 不可數時通常指時間本身；`times` 可數時通常指次數。
- 這類詞不適合只用中文單複數直覺去理解，要直接記整個用法。
...
### Topic 摘要
老師用學生第一次正式帶 training 的情境，帶出「新手」與「熟悉工作門道」的表達。

### 老師補充重點
- `beginner` 較中性。
- `noob` 很口語，常見於遊戲或朋友聊天。
- `green` 指缺乏經驗，語氣較口語。
- `learn the ropes` 指真正熟悉工作怎麼運作，知道哪些做法有效、哪些沒用。
...
### Topic 摘要
課堂中分享了大量使用 AI 協助備課、整理內容與產出教材的經驗，老師也回應不同工具的使用方式與優缺點。

### 老師補充重點
- AI 可以幫忙整理 `syllabus`、課程內容、教材草稿與活動設計。
- 問題越具體，AI 的幫助通常越大。
- `precise prompts` 和 `good questions` 會直接影響結果品質。
...
### Topic 摘要
老師詢問學生最近如何練英文，並談到 podcast、NHK News、英語學習型節目，以及英聽訓練方式。

### 老師補充重點
- 英聽時若先在腦中翻譯成中文，速度常常跟不上。
- 多聽固定主題的素材，有助於累積熟悉的語境與字彙。
- 可以從短版新聞或有教學導向的 podcast 開始。

### 關鍵單字／片語
- `podcast`：播客
- `listening comprehension`：聽力理解
- `translate in my head`：在腦中翻譯
...
### Topic 摘要
課堂中聊到童年遊戲、動畫、零食、懷舊感，以及企業如何利用 nostalgia 來吸引消費者。

### 老師補充重點
- `nostalgic` 是形容詞，`nostalgia` 是名詞。
- `cater to` 不只可用於餐飲，也可指「迎合需求、偏好、市場」。
- `cater to people’s nostalgia` 是很自然的說法。

### 關鍵單字／片語
- `nostalgic / nostalgia`：懷舊的／懷舊感
- `cater to`：迎合、滿足需求
- `popsicle`：冰棒
- `soft serve`：霜淇淋
...
### Topic 摘要
後半堂課進入講義 `New Products and Services`，主題是未來住宅、IoT 與新科技如何改變生活。

### 老師補充重點
- `home is where the heart is` 是常見片語；講義改成 `home is where the IoT is` 是雙關。
- `household` 可指家庭、家戶，也可指家庭中的人與住家整體概念。
- `IoT` 是 `Internet of Things`，強調家中裝置彼此連網、可遠端控制。
...
### Topic 摘要
老師順著科技主題補充一組很實用的創新科技字彙，並特別說明細微語感差異。

### 老師補充重點
- `experiment` 是實驗，也可指「試做、試試看」。
- `augment` 是「加上一層、擴增」。
- `enhance` 偏向「提升品質、效果、功能」。
- `complement` 是「補足、互補」，不是 `compliment`。
- `evolve` 指隨時間逐漸演變。

### 關鍵單字／片語
- `augmented reality (AR)`：擴增實境
- `augment / augmentation`：擴增；增強
- `enhance / enhancement`：提升；增進
- `complement`：補充；互補
- `evolve / evolution`：演變
- `efficiency`：效率
- `explore`：探索
...
### Topic 摘要
老師整理居住相關詞彙，區分同房、同屋、同公寓等不同情境，也延伸到 `dwelling` 這種較正式的說法。

### 老師補充重點
- `roommate`：同房室友
- `housemate`：同屋但不一定同房
- `flatmate`：英式英文，常指同公寓室友
- `dwelling`：較正式，指住宅、居所

### 關鍵單字／片語
- `dwelling / dwell`：住宅／居住
- `roommate`：同房室友
- `housemate`：同屋不同房
- `flatmate`：公寓室友
...
### Topic 摘要
老師最後留下思考題，請學生下次討論智慧住宅的優缺點，以及理想中的 future home。

### 老師補充重點
- 常見討論方向是 `pros and cons`。
- 智慧住宅可從方便性、效率、安全、環保，以及資安風險、成本、故障依賴來談。
- `eco-friendly` 是這類主題很好用的形容詞。
- `fulfill` 可用在「實現理想住宅需求」這類句子裡。
...
- `Language of instruction`：教學語言
- `Syllabus`：課程大綱
- `Beginner / noob`：初學者／菜鳥
- `Learn the ropes / know the ropes`：掌握竅門；熟悉運作
...
- `Labour Day`：勞動節
- `Upcoming`：即將到來的
- `Nostalgic / nostalgia`：懷舊的／懷舊之情

### 實用商業與生活動詞
- `Cater / cater to`：提供飲食；迎合需求
- `Fulfill`：實現；履行
- `Pros and cons`：優缺點
- `Eco-friendly`：環保的
...
- `bread` 通常不可數；若是一整條可說 `a loaf of bread`
- `people` 可指人們，也可指民族；`peoples` 常指多個族群
- `time` 是時間；`times` 是次數
- `teacher` 可以用，但職場訓練常更自然說 `instructor`
- `class` 可以用，但公司內訓、專業課程常更自然用 `training`
- `nostalgic` 是形容詞，`nostalgia` 是名詞
- `complement` 和 `compliment` 不同，不要混淆
- `cater to + needs/preferences/nostalgia` 是高頻搭配
- `learn the ropes` 是固定片語，不要逐字直譯
- `home is where the heart is` 是固定說法，講義中的 `home is where the IoT is` 是改寫雙關
- `solar` 和 `lunar` 分別對應太陽與月亮相關概念

## 本週表達修正重點

- 回答問題時不要只給很短的骨架句，盡量補背景和結果。
- 避免中式直譯，例如 `I'm not go anywhere`，要改成完整時態句。
- 科技主題回答時，可以多用 `improve`、`enhance`、`efficient`、`eco-friendly`、`convenient` 這類高頻字。
- 談工作經驗時，可多用 `I'm still learning`、`I'm getting used to it`、`I need time to learn the ropes` 這種自然表達。
- 聽力練習主題要熟記 `listening comprehension`、`translate in my head`、`natural speech`、`practical advice` 等說法。
- `people / peoples`、`time / times` 要直接用例句去記，不要只背中文意思。
